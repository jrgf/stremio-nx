...
void nx_canvas_context_2d_draw_image(const FunctionCallbackInfo<Value> &info) {
	...
	if (img && img->logical_width && img->logical_height) {
		source_w = img->logical_width;
		source_h = img->logical_height;
	}
	...
	if (!(sw && sh && dw && dh))
		return;
	// drawImage source coordinates use intrinsic pixels, even for a resized video.
	const double scale_x = image->width() / source_w, scale_y = image->height() / source_h;
	sx *= scale_x; sw *= scale_x;
	sy *= scale_y; sh *= scale_y;
