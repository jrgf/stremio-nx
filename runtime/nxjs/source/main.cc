...
extern "C" size_t horizon_mman_data_used_size(void);
extern "C" size_t horizon_mman_data_committed_size(void);
...
// log the reason and heap stats, then terminate all threads in application
// mode. Applet mode retains the resource cleanup + hbloader exit handshake.
// ---------------------------------------------------------------------------
static void nx_v8_fatal_exit(const char *kind, const char *location,
	...
	fflush(stderr);
	// stderr's startup log is truncated on the next launch. Keep fatal evidence
	// separately so reopening the app cannot erase the allocation failure.
	FILE *fatal = fopen("sdmc:/switch/nxjs-fatal.log", "a");
	if (fatal) {
		fprintf(fatal, "FATAL %s at %s: %s\n"
		        "native used=%zu KiB total=%zu KiB; data arena live=%zu KiB committed=%zu KiB\n",
		        kind, location ? location : "?", detail ? detail : "",
		        (size_t)mi.uordblks / 1024,
		        (size_t)(fake_heap_end - fake_heap_start) / 1024,
		        horizon_mman_data_used_size() / 1024,
		        horizon_mman_data_committed_size() / 1024);
		fflush(fatal);
		fsync(fileno(fatal));
		fclose(fatal);
	}
	...
	if (applet_type == AppletType_Application || applet_type == AppletType_SystemApplication) {
		// A fatal isolate cannot run JS shutdown. End all process threads together:
		// returning to hbloader would unmap code while audio/decoder workers run.
		svcExitProcess();
	}
