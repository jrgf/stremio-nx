...
struct nx_media {
	...
	AVFrame *download_frame = nullptr;
	std::atomic<int> decoder_mode{0}; // 0 software, 1 NVDEC requested, 2 hardware frames received
	AVCodecContext *actx = nullptr;
	AVCodecContext *sctx = nullptr;
	int vstream = -1;
	int astream = -1;
	int sstream = -1;
	std::vector<nx_media_track> tracks;
	std::atomic<int> selected_audio{-1}, selected_subtitle{-1};
	std::atomic<int> requested_audio{-2}, requested_subtitle{-2};
	struct subtitle_cue { double start, end; std::string text; };
	std::mutex subtitle_mutex;
	std::deque<subtitle_cue> subtitles;
	...
	std::atomic<int64_t> transfer_peak_us{0}, convert_peak_us{0};
	double duration = 0;
...
AVPixelFormat select_video_format(AVCodecContext *ctx, const AVPixelFormat *formats) {
	auto *m = static_cast<nx_media *>(ctx->opaque);
	for (const AVPixelFormat *p = formats; *p != AV_PIX_FMT_NONE; ++p) {
		if (*p == m->hw_format) {
			m->decoder_mode.store(1);
			return *p;
		}
	}
	// FFmpeg calls again without the hardware format if the profile/setup fails.
	m->decoder_mode.store(0);
	fprintf(stderr, "[video] NVDEC unavailable for this stream; using software decode\n");
	for (const AVPixelFormat *p = formats; *p != AV_PIX_FMT_NONE; ++p) {
...
void configure_nvdec(nx_media *m, const AVCodec *codec) {
	...
	if (!config) {
		fprintf(stderr, "[video] software decode: %s has no available NVTEGRA configuration\n", codec->name);
		return;
	}
	const int ret = av_hwdevice_ctx_create(&m->vctx->hw_device_ctx, type, nullptr, nullptr, 0);
	if (ret < 0) {
		char error[128];
		av_strerror(ret, error, sizeof(error));
		fprintf(stderr, "[video] NVDEC device creation failed: %s; using software decode\n", error);
		av_buffer_unref(&m->vctx->hw_device_ctx);
		return;
	}
	m->hw_format = config->pix_fmt;
	m->decoder_mode.store(1);
	m->vctx->opaque = m;
	m->vctx->get_format = select_video_format;
	m->vctx->thread_count = 1; // NVDEC owns the decode work; avoid CPU frame-worker copies.
	m->vctx->extra_hw_frames = 2;
	fprintf(stderr, "[video] requesting NVDEC via NVTEGRA for %s\n", codec->name);
}

int allocate_download_frame(AVFrame *frame, AVPixelFormat format, int width, int height) {
	av_frame_unref(frame);
	if (width <= 0 || height <= 0 || width > 4096 || height > 4096) return AVERROR(EINVAL);
	// VIC needs each plane address and pitch aligned to 256 bytes. av_malloc's
	// default alignment is insufficient; retain the original allocation in buf[0].
	const int size = av_image_get_buffer_size(format, width, FFALIGN(height, 32), 256);
	if (size < 0) return size;
	frame->buf[0] = av_buffer_alloc((size_t)size + 255);
	if (!frame->buf[0]) return AVERROR(ENOMEM);
	uint8_t *aligned = reinterpret_cast<uint8_t *>((reinterpret_cast<uintptr_t>(frame->buf[0]->data) + 255) & ~uintptr_t(255));
	const int ret = av_image_fill_arrays(frame->data, frame->linesize, aligned, format, width, FFALIGN(height, 32), 256);
...
void record_frame_time(std::atomic<int64_t> &peak, std::chrono::steady_clock::time_point start) {
	const int64_t us = std::chrono::duration_cast<std::chrono::microseconds>(std::chrono::steady_clock::now() - start).count();
	int64_t previous = peak.load(std::memory_order_relaxed);
	while (us > previous && !peak.compare_exchange_weak(previous, us, std::memory_order_relaxed)) {}
}
...
bool enqueue_video(nx_media *m, AVFrame *frame, double pts) {
	...
	if (frame->hw_frames_ctx) {
		...
		if (!m->download_frame) m->download_frame = av_frame_alloc();
		if (!m->download_frame) { set_fatal(m, "hardware transfer frame allocation failed", AVERROR(ENOMEM)); return false; }
		AVFrame *sw = m->download_frame;
		if (sw->width != frame->width || sw->height != frame->height || sw->format != frames->sw_format) {
			const int ret = allocate_download_frame(sw, frames->sw_format, frame->width, frame->height);
			if (ret < 0) { set_fatal(m, "hardware transfer buffer allocation failed", ret); return false; }
		}
		const int ret = av_hwframe_transfer_data(sw, frame, 0);
		record_frame_time(m->transfer_peak_us, transfer_start);
		if (ret < 0) { set_fatal(m, "NVDEC frame transfer failed", ret); return false; }
		if (m->decoder_mode.exchange(2) != 2) fprintf(stderr, "[video] NVDEC active: %dx%d\n", frame->width, frame->height);
		// Keep the existing BGRA presentation ring. Hardware decoding is active;
		// downloading/color conversion still feeds the current canvas renderer.
		frame = sw;
	}

	const auto convert_start = std::chrono::steady_clock::now();
	m->sws = sws_getCachedContext(m->sws, frame->width, frame->height,
	                              (AVPixelFormat)frame->format, m->render_width,
	                              m->render_height, AV_PIX_FMT_BGRA, SWS_BILINEAR,
	                              NULL, NULL, NULL);
	if (!m->sws) {
	...
	int dst_stride[4] = {m->render_width * 4, 0, 0, 0};
	const int rows = sws_scale(m->sws, frame->data, frame->linesize, 0, frame->height, dst, dst_stride);
	record_frame_time(m->convert_peak_us, convert_start);
	if (rows != m->render_height) { set_fatal(m, "video frame conversion failed", rows < 0 ? rows : AVERROR(EINVAL)); return false; }
	slot->pts = pts;
...
bool container_seek(nx_media *m, double to_seconds) {
	...
	if (m->sctx)
		avcodec_flush_buffers(m->sctx);
	{
		std::lock_guard<std::mutex> lock(m->subtitle_mutex);
		m->subtitles.clear();
	}
	if (m->swr) {
...
void change_track(nx_media *m, int id, bool subtitle) {
	AVCodecContext *next = nullptr;
	if (id >= 0) {
		AVCodecParameters *par = m->fmt->streams[id]->codecpar;
		const AVCodec *codec = avcodec_find_decoder(par->codec_id);
		next = avcodec_alloc_context3(codec);
		if (!next || avcodec_parameters_to_context(next, par) < 0 ||
		    avcodec_open2(next, codec, nullptr) < 0) {
			avcodec_free_context(&next);
			std::lock_guard<std::mutex> lock(m->subtitle_mutex);
			m->track_error = "Unable to open the selected track; previous track retained.";
			return;
		}
	}
	if (subtitle) {
		avcodec_free_context(&m->sctx);
		m->sctx = next;
		m->sstream = id;
		m->selected_subtitle.store(id);
	} else {
		avcodec_free_context(&m->actx);
		m->actx = next;
		m->astream = id;
		m->selected_audio.store(id);
	}
}

void do_seek(nx_media *m, double *seek_drop_until) {
	double target = m->seek_target.load();
	int audio = m->requested_audio.exchange(-2);
	int subtitle = m->requested_subtitle.exchange(-2);
	if (audio != -2) change_track(m, audio, false);
	if (subtitle != -2) change_track(m, subtitle, true);
...
// FFmpeg converts text subtitles to ASS rectangles. Render their text only;
// bitmap tracks are advertised as unsupported, without allocating image cues.
std::string subtitle_plain_text(const AVSubtitleRect *rect) {
	if (rect->text) return std::string(rect->text).substr(0, 4096);
	if (!rect->ass) return {};
	...
	std::string out;
	bool tag = false;
	for (; *p && out.size() < 4096; ++p) {
		if (*p == '{') { tag = true; continue; }
		if (*p == '}') { tag = false; continue; }
		if (tag) continue;
...
void decode_subtitle(nx_media *m, AVPacket *pkt, double seek_drop_until) {
	AVSubtitle sub = {};
	int got = 0;
	if (avcodec_decode_subtitle2(m->sctx, &sub, &got, pkt) < 0 || !got) {
		avsubtitle_free(&sub);
		return;
	}
	double pts = sub.pts != AV_NOPTS_VALUE ? (double)sub.pts / AV_TIME_BASE
	    : pkt->pts != AV_NOPTS_VALUE ? pkt->pts * av_q2d(m->fmt->streams[m->sstream]->time_base) : 0;
	double start = pts + sub.start_display_time / 1000.0 + m->loop_offset;
	double end = pts + sub.end_display_time / 1000.0 + m->loop_offset;
	if (end <= start && pkt->duration > 0) end = start + pkt->duration * av_q2d(m->fmt->streams[m->sstream]->time_base);
	std::string text;
	for (unsigned i = 0; i < sub.num_rects && text.size() < 4096; ++i) {
		std::string part = subtitle_plain_text(sub.rects[i]);
		if (!part.empty()) { if (!text.empty()) text += '\n'; text += part; }
	}
	avsubtitle_free(&sub);
	if (!text.empty() && end > start && (seek_drop_until < 0 || end > seek_drop_until)) {
		std::lock_guard<std::mutex> lock(m->subtitle_mutex);
		// Bounded even for malformed files with thousands of overlapping cues.
		if (m->subtitles.size() >= 128) m->subtitles.pop_front();
		m->subtitles.push_back({start, end, text.substr(0, 4096)});
	}
}
...
void decode_thread_main(nx_media *m) {
	...
	while (!m->quit.load()) {
		...
		} else if (pkt->stream_index == m->sstream && m->sctx) {
			decode_subtitle(m, pkt, seek_drop_until);
		}
		av_packet_unref(pkt);
	}
...
nx_media_t *nx_media_open(const char *path, const uint8_t *mem,
                          ...
                          size_t errbuf_size, int max_width, int max_height) {
	if (max_width < 0 || max_height < 0 || max_width > 4096 || max_height > 4096 || ((max_width == 0) != (max_height == 0))) {
		snprintf(errbuf, errbuf_size, "invalid video render bounds");
		return nullptr;
	}
	...
	if (m->vstream >= 0) {
		...
		const double scale = max_width ? std::min({1.0, (double)max_width / m->width, (double)max_height / m->height}) : 1.0;
		m->render_width = std::max(1, (int)std::floor(m->width * scale));
		m->render_height = std::max(1, (int)std::floor(m->height * scale));
		fprintf(stderr, "[video] source %dx%d -> render %dx%d\n", m->width, m->height, m->render_width, m->render_height);
		for (int i = 0; i < RING_SLOTS; i++) {
			m->slots[i].bgra =
			    (uint8_t *)malloc((size_t)m->render_width * m->render_height * 4);
			if (!m->slots[i].bgra) {
				snprintf(errbuf, errbuf_size, "out of memory");
				goto fail;
			}
	...
	for (unsigned i = 0; i < m->fmt->nb_streams; ++i) {
		AVStream *stream = m->fmt->streams[i];
		AVCodecParameters *par = stream->codecpar;
		if (par->codec_type != AVMEDIA_TYPE_AUDIO && par->codec_type != AVMEDIA_TYPE_SUBTITLE) continue;
		const AVCodecDescriptor *desc = avcodec_descriptor_get(par->codec_id);
		bool subtitle = par->codec_type == AVMEDIA_TYPE_SUBTITLE;
		bool supported = avcodec_find_decoder(par->codec_id) && (!subtitle || (desc && (desc->props & AV_CODEC_PROP_TEXT_SUB)));
		...
		m->tracks.push_back({(int)i, subtitle, supported,
		    language ? std::string(language->value).substr(0, 64) : "und",
		    title ? std::string(title->value).substr(0, 256) : "",
		    avcodec_get_name(par->codec_id)});
	}
	m->selected_audio.store(m->astream);
...
int nx_media_render_width(nx_media_t *m) { return m->render_width; }
int nx_media_render_height(nx_media_t *m) { return m->render_height; }
void nx_media_frame_times(nx_media_t *m, double *transfer_ms, double *convert_ms) {
	*transfer_ms = m->transfer_peak_us.exchange(0, std::memory_order_relaxed) / 1000.0;
	*convert_ms = m->convert_peak_us.exchange(0, std::memory_order_relaxed) / 1000.0;
}
double nx_media_duration(nx_media_t *m) { return m->duration; }
bool nx_media_has_audio(nx_media_t *m) { return m->selected_audio.load() >= 0; }
bool nx_media_has_video(nx_media_t *m) { return m->vstream >= 0; }
const char *nx_media_video_decoder(nx_media_t *m) {
	switch (m->decoder_mode.load()) {
		case 1: return "nvdec-pending";
		case 2: return "nvdec";
		default: return "software";
	}
}

const std::vector<nx_media_track> &nx_media_tracks(nx_media_t *m) { return m->tracks; }
int nx_media_audio_track(nx_media_t *m) { return m->selected_audio.load(); }
int nx_media_subtitle_track(nx_media_t *m) { return m->selected_subtitle.load(); }

bool nx_media_select_track(nx_media_t *m, int id, bool subtitle) {
	bool valid = subtitle && id == -1;
	for (const auto &track : m->tracks)
		if (track.id == id && track.subtitle == subtitle && track.supported) valid = true;
	if (!valid) return false;
	{
		std::lock_guard<std::mutex> lock(m->subtitle_mutex);
		m->track_error.clear();
		if (subtitle) m->subtitles.clear();
	}
	if (subtitle) m->requested_subtitle.store(id);
	else m->requested_audio.store(id);
	nx_media_seek(m, nx_media_current_time(m));
	return true;
}

std::string nx_media_track_error(nx_media_t *m) {
	std::lock_guard<std::mutex> lock(m->subtitle_mutex);
	return m->track_error;
}

std::string nx_media_subtitle_text(nx_media_t *m, double seconds) {
	if (m->seeking.load() || !std::isfinite(seconds)) return {};
	std::lock_guard<std::mutex> lock(m->subtitle_mutex);
	std::string text;
	for (auto it = m->subtitles.begin(); it != m->subtitles.end();) {
		// Retain a small history for subtitle delay adjustments.
		if (it->end < seconds - 60) { it = m->subtitles.erase(it); continue; }
		if (it->start <= seconds && it->end > seconds && text.size() < 4096) {
			if (!text.empty()) text += '\n';
			text += it->text;
		}
		++it;
...
void nx_media_destroy(nx_media_t *m) {
	m->quit.store(true);
	// A blocked source read cannot observe quit until it is woken.
	if (m->source) nx_media_source_close(m->source);
	m->ctl_cv.notify_all();
	...
	av_frame_free(&m->download_frame);
	if (m->actx)
		avcodec_free_context(&m->actx);
	if (m->sctx)
		avcodec_free_context(&m->sctx);
