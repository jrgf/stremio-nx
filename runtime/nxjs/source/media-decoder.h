...
// (the decode thread reads from it for the media's whole lifetime). Optional
// render bounds reduce presentation buffers without changing source metadata;
// zero/zero preserves the source size. The aspect ratio is preserved.
nx_media_t *nx_media_open(const char *path, const uint8_t *mem,
                          ...
                          size_t errbuf_size, int max_width = 0, int max_height = 0);

// Metadata (valid after a successful open).
int nx_media_width(nx_media_t *m);
int nx_media_height(nx_media_t *m);
int nx_media_render_width(nx_media_t *m);
int nx_media_render_height(nx_media_t *m);
// Peak hardware transfer wait and scale/color-conversion time since the last read.
...
// Reports actual frame delivery, not merely the requested decoder configuration.
const char *nx_media_video_decoder(nx_media_t *m);

struct nx_media_track {
	int id;
	bool subtitle;
	bool supported;
	std::string language;
...
// Immutable stream metadata; selections are applied by the decode thread.
const std::vector<nx_media_track> &nx_media_tracks(nx_media_t *m);
int nx_media_audio_track(nx_media_t *m);
int nx_media_subtitle_track(nx_media_t *m);
...
// into `*buffer_inout` (a caller-owned render_width*render_height*4 BGRA buffer; the
